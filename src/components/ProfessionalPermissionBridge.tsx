    ...
    <div className="delegated-member-layout">
      ...
      <main>
        {inviteOpen ? <form className="delegated-member-form" onSubmit={(event) => void addMember(event)}>
          ...
          {canManageRoles && <fieldset><legend>Cargos profissionais</legend><div className="delegated-role-list">{roles.map((role) => { const checked = draft.jobRoles.some((item) => item.id === role.id); return <label key={role.id} className={checked ? "active" : ""}><input type="checkbox" checked={checked} onChange={() => setDraft({ ...draft, jobRoles: checked ? draft.jobRoles.filter((item) => item.id !== role.id) : [...draft.jobRoles, role] })} /><span style={{ "--role-color": role.color } as React.CSSProperties} /><b>{role.name}</b></label>; })}</div></fieldset>}
