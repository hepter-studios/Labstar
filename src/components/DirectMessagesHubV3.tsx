  ...
  return (
    <section className={`direct-hub ${selectedContact ? "conversation-open" : ""}`}>
      ...
      <aside className="dm-sidebar">
        ...
        <div className="dm-list-heading"><span>MENSAGENS DIRETAS</span><button onClick={startNewMessage} title="Nova mensagem"><Plus size={15} /></button></div>
      ...
      {selectedContact ? (
        <DirectConversation
          ...
          onStartCall={() => onOpenWorkspace(firstVoiceChannelId ?? undefined)}
        />
      ) : (
        <>
          <main className="dm-home-main">
            <header className="dm-home-header"><div><Users size={20} /><strong>Mensagens diretas</strong></div><div><button title="Nova mensagem" onClick={startNewMessage}><Pencil size={16} /></button><button title="Nova conversa" onClick={startNewMessage}><UserPlus size={16} /></button></div></header>
...
function InboxPanel({ inboxTab, setInboxTab, items, unreadCount, mentionCount, onMarkAll, onOpenItem }: {
  ...
  return <aside className="dm-inbox">
    <header><div><Inbox size={19} /><strong>Caixa de entrada</strong></div><div><button title="Marcar tudo como lido" onClick={onMarkAll}><Check size={15} /></button></div></header>
    ...
    <div className="dm-inbox-list">{visible.map((item,index) => <article key={item.id} className={item.channelId ? "clickable" : ""} onClick={() => onOpenItem(item)}><span className="dm-inbox-icon">{index % 2 ? <Bell size={18} /> : <AtSign size={18} />}</span><div><strong>{item.title}</strong><small>{item.body}</small><em>{item.channelId ? "Abrir canal" : index % 2 ? "Atualização" : "Mensagem"}</em></div><time>{formatCompactTime(item.createdAt)}</time>{!item.isRead && <b>1</b>}</article>)}{!items.length && <div className="dm-inbox-empty"><Inbox size={25} /><strong>Tudo em dia</strong><span>Nenhuma mensagem pendente nesta caixa.</span></div>}</div>
    <button className="dm-inbox-all" onClick={() => setShowAll((value) => !value)}>{showAll ? "Mostrar menos" : "Ver todas as mensagens"}</button>
  </aside>;
}

function DirectConversation({ member, contact, threadId, dmAvailable, notice, onThreadsChanged, onStartCall }: {
  member: Member;
  contact: Member;
  ...
  const visibleMessages = messages.filter((message) => {
    if (pinnedOnly && !message.isPinned) return false;
    return !search.trim() || `${message.body} ${message.author?.name ?? ""}`.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase());
  });
  ...
  return <>
    <main className="dm-conversation">
      ...
      <div className="dm-message-scroll">
        ...
        {!loading && threadId && !visibleMessages.length && <div className="dm-thread-empty"><MessageSquare size={25} /><strong>{pinnedOnly ? "Nenhuma mensagem fixada" : "Nenhuma mensagem ainda"}</strong><span>{pinnedOnly ? "Fixe mensagens importantes para encontrá-las aqui." : `Envie a primeira mensagem para ${contact.name}.`}</span></div>}
