...
export function DirectMessagesHub({ member, onOpenWorkspace }: Props) {
  ...
  async function refreshThreads(silent = false) {
    try {
      ...
      if (!silent) setNotice("Mensagens privadas ainda não estão disponíveis no banco deste ambiente.");
    }
  }
  ...
  useEffect(() => {
    ...
    void (async () => {
      const [teamResult, collaborationResult, notificationResult] = await Promise.allSettled([
        listMembers(), loadCollaboration(), listNotifications(member.id),
      ]);
      if (cancelled) return;
  ...
  const allContacts = useMemo<Contact[]>(() => members
    ...
    .map((item) => ({ member: item, thread: threadByMember.get(item.id) ?? null })), [members, member.id, threadByMember]);
  ...
  const homeContacts = useMemo(() => {
    ...
    return [...activeContacts].sort((a, b) => {
      const aScore = (a.thread?.unreadCount ?? 0) * 100 + (favoriteIds.has(a.member.id) ? 25 : 0) + (a.thread ? 10 : 0);
      const bScore = (b.thread?.unreadCount ?? 0) * 100 + (favoriteIds.has(b.member.id) ? 25 : 0) + (b.thread ? 10 : 0);
  ...
  async function openContact(contact: Contact) {
    if (contact.member.status !== "active") {
      setNotice("Este membro ainda está pendente e não pode receber mensagens privadas.");
      return;
    }
    ...
    if (!dmAvailable) {
      ...
      setNotice("As mensagens privadas ainda não estão disponíveis neste ambiente.");
      return;
    }
    try {
      ...
      setDmAvailable(false);
      setSelectedThreadId(null);
      setNotice("Não foi possível criar a conversa privada agora.");
    }
  }
  ...
  async function markInboxRead() {
    ...
    } catch { setNotice("Não foi possível marcar a caixa de entrada como lida."); }
  }
  ...
  return <section className={`direct-hub dm-v4 ${selectedContact ? "conversation-open" : ""}`}>
    <aside className="dm-space-rail" aria-label="Labstar e espaços">
      <button className="dm-home-mark active" onClick={goHome} title="Mensagens diretas" aria-label="Mensagens diretas"><img className="labstar-dm-logo" src="/labstar-dm.svg" alt="" aria-hidden="true" /><i /></button>
      <div className="dm-space-list">{spaces.map((space) => <button key={space.id} title={space.name} style={{ "--space-color": space.color } as React.CSSProperties} onClick={() => onOpenWorkspace(firstChannels[space.id])}>{space.logoUrl ? <img src={space.logoUrl} alt="" /> : <span>{space.icon || "★"}</span>}</button>)}</div>
    ...
    <aside className="dm-sidebar">
      ...
      <div className="dm-list-heading"><span>{directoryTab === "pending" ? "ACESSOS PENDENTES" : "MENSAGENS DIRETAS"}</span><button onClick={startNewMessage} title="Nova mensagem"><Plus size={15} /></button></div>
...
function DirectConversation({ member, contact, threadId, dmAvailable, notice, favorite, onToggleFavorite, onThreadsChanged, onStartCall }: { member: Member; contact: Member; threadId: string | null; dmAvailable: boolean; notice: string; favorite: boolean; onToggleFavorite: () => void; onThreadsChanged: () => void; onStartCall: () => void }) {
  ...
  useEffect(() => {
    setLoading(Boolean(threadId)); setDraft(""); setFiles([]); setError(""); setPinnedOnly(false);
  ...
  async function submit(event: React.FormEvent) {
    event.preventDefault(); if (!threadId || (!draft.trim() && !files.length)) return;
    setSending(true); setError("");
    try { await sendDirectMessage({ threadId, authorId: member.id, body: draft, files }); setDraft(""); setFiles([]); await refresh(true); onThreadsChanged(); }
