...
if (!supabaseUrl || !supabaseAnonKey) {
  ...
  supabaseUrl ||= javascript.match(/https:\/\/[a-z0-9-]+\.supabase\.co/)?.[0];
  supabaseAnonKey ||= javascript.match(/eyJ[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+/)?.[0];
}

if (!supabaseUrl || !supabaseAnonKey) throw new Error("Public Supabase configuration was not found");
await appendFile(output, `VITE_SUPABASE_URL=${supabaseUrl}\nVITE_SUPABASE_ANON_KEY=${supabaseAnonKey}\n`);
