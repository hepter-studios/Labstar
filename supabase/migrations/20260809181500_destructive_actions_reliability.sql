-- Labstar — ações destrutivas confiáveis.
-- Reinstala e endurece as rotinas de limpar chats e excluir contas.
-- É idempotente e pode ser aplicada novamente com segurança.
...
begin
  ...
  -- Remove primeiro os binários do bucket; o metadado de anexos cai em cascata
  -- junto das mensagens logo abaixo.
...
begin
  ...
  if target.id is not null then
    ...
    -- A exclusão permanente não depende mais de uma segunda ida manual à tela:
    -- a própria transação bloqueia o acesso antes de anonimizar e apagar o login.
...
comment on function public.clear_channel_chat(uuid) is
  'Apaga mensagens e arquivos de um canal para administradores e cargos com manage_channels.';
comment on function public.clear_direct_conversation(uuid) is
  'Apaga mensagens e arquivos de uma conversa direta para qualquer participante ativo.';
comment on function public.delete_labstar_account(text, text) is
  'Bloqueia, anonimiza e exclui a identidade Auth de uma conta nao-owner com confirmacao exata do e-mail.';
