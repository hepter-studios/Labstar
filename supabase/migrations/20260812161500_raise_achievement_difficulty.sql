-- Preserva as três conquistas de onboarding e torna raras/épicas marcos reais.
-- Conquistas já obtidas não são revogadas quando o alvo aumenta.

begin;

-- Estas missões eram conceitualmente impossíveis no contexto correto: a jornada
-- de conquistas só existe depois que o usuário já entrou em uma organização.
delete from public.member_achievements as achievement
where achievement.achievement_key in ('organization_founder', 'universe_architect');
...
begin
  ...
  -- Usar a constraint evita colisão entre a coluna achievement_key e o parâmetro
  -- de saída homônimo da função PL/pgSQL.
