...
begin
  ...
  values (public.current_member_id(), trim(p_question_1), crypt(lower(trim(p_answer_1)), gen_salt('bf')), trim(p_question_2), crypt(lower(trim(p_answer_2)), gen_salt('bf')), now())
...
begin
  select * into q from public.member_security_questions where member_id=public.current_member_id();
...
begin
  select * into q from public.member_security_questions where member_id=public.current_member_id() for update;
  ...
  if valid then
    update public.member_security_questions set failed_attempts=0, locked_until=null, last_verified_at=now(), updated_at=now() where member_id=public.current_member_id();
  ...
  update public.member_security_questions set failed_attempts=case when failures>=5 then 0 else failures end,
    locked_until=next_lock, updated_at=now() where member_id=public.current_member_id();
...
create table if not exists public.direct_messages (
  ...
  author_id uuid not null references public.members(id) on delete cascade default public.current_member_id(),
...
-- Corrige instalações que executaram a primeira revisão da v9.
alter table public.direct_messages alter column author_id set default public.current_member_id();
...
returns boolean language sql stable security definer set search_path=public,pg_temp as $$
  select exists(select 1 from public.direct_conversation_members where conversation_id=p_conversation_id and member_id=public.current_member_id())
$$;
...
create policy direct_messages_insert on public.direct_messages for insert to authenticated with check(author_id=public.current_member_id() and public.is_direct_conversation_member(conversation_id));
drop policy if exists direct_messages_update on public.direct_messages;
create policy direct_messages_update on public.direct_messages for update to authenticated using(author_id=public.current_member_id()) with check(author_id=public.current_member_id());
drop policy if exists direct_messages_delete on public.direct_messages;
create policy direct_messages_delete on public.direct_messages for delete to authenticated using(author_id=public.current_member_id());
...
begin
  if p_other_member_id=public.current_member_id() then raise exception 'cannot_message_self'; end if;
  ...
  key := least(public.current_member_id()::text,p_other_member_id::text)||':'||greatest(public.current_member_id()::text,p_other_member_id::text);
  ...
  insert into public.direct_conversation_members(conversation_id,member_id) values(conversation,public.current_member_id()),(conversation,p_other_member_id) on conflict do nothing;
  return conversation;
end $$;
