...
use crate::{auth::AuthenticatedMember, error::ApiError, state::{AppState, BackendEvent}};
...
pub async fn update_work_item(
    ...
    if let Some(kind) = &input.kind { validate_kind(kind)?; }
    if let Some(status) = &input.status { validate_status(status)?; }
    if let Some(priority) = &input.priority { validate_priority(priority)?; }
    let mut updated = None;
    mutate_items(&state, |items| {
        if let Some(item) = items.iter_mut().find(|item| item.id == item_id) {
            if let Some(title) = &input.title {
                item.title = clean_required(title, 180, "title").unwrap_or_else(|_| item.title.clone());
            }
            if let Some(details) = &input.details { item.details = details.trim().chars().take(12_000).collect(); }
            if let Some(kind) = &input.kind { item.kind = kind.clone(); }
            if let Some(status) = &input.status {
                item.status = status.clone();
                item.completed_at = if status == "done" { item.completed_at.or(Some(Utc::now())) } else { None };
            }
            if let Some(priority) = &input.priority { item.priority = priority.clone(); }
            if let Some(value) = input.channel_id { item.channel_id = value; }
            if let Some(value) = input.space_id { item.space_id = value; }
            if let Some(value) = input.assignee_id { item.assignee_id = value; }
            if let Some(value) = input.due_at { item.due_at = value; }
...
pub async fn delete_work_item(
    ...
    if !removed { return Err(ApiError::NotFound("work_item")); }
    state.publish(BackendEvent::WorkItemsChanged);
...
async fn load_items(state: &AppState) -> Result<Vec<WorkItem>, ApiError> {
    ...
    match value {
        Some(Value::Array(items)) => Ok(items.into_iter().filter_map(|value| serde_json::from_value(value).ok()).collect()),
        _ => Ok(Vec::new()),
    }
}

async fn mutate_items<F>(state: &AppState, mutate: F) -> Result<(), ApiError>
where F: FnOnce(&mut Vec<WorkItem>) {
    let mut tx = state.pool.begin().await?;
    let current = sqlx::query_scalar::<_, Value>("select nodes from public.workspaces where id=$1 for update")
        .bind(WORKSPACE_ID).fetch_optional(&mut *tx).await?;
    let mut items = match current {
        Some(Value::Array(values)) => values.into_iter().filter_map(|value| serde_json::from_value(value).ok()).collect(),
        _ => Vec::new(),
    };
    mutate(&mut items);
    ...
    "#).bind(WORKSPACE_ID).bind(nodes).execute(&mut *tx).await?;
    tx.commit().await?;
    Ok(())
}
...
fn sort_items(items: &mut [WorkItem]) {
    items.sort_by(|a,b| {
        ...
        if done != std::cmp::Ordering::Equal { return done; }
...
fn validate_kind(value: &str) -> Result<(), ApiError> {
    matches!(value, "task" | "decision" | "follow_up").then_some(()).ok_or(ApiError::invalid("kind"))
}
fn validate_status(value: &str) -> Result<(), ApiError> {
    matches!(value, "open" | "in_progress" | "blocked" | "done").then_some(()).ok_or(ApiError::invalid("status"))
}
fn validate_priority(value: &str) -> Result<(), ApiError> {
    matches!(value, "low" | "medium" | "high" | "urgent").then_some(()).ok_or(ApiError::invalid("priority"))
}
...
fn clean_optional(value: Option<String>, limit: usize) -> String {
    value.unwrap_or_default().trim().chars().take(limit).collect()
}
