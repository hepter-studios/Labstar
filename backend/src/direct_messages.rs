...
#[serde(rename_all = "camelCase")]
pub struct CreateThreadInput { pub other_member_id: Uuid }

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SendMessageInput { pub body: String, pub reply_to: Option<Uuid> }

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct UpdateMessageInput { pub body: Option<String>, pub is_pinned: Option<bool> }
...
pub async fn list_threads(State(state): State<AppState>, member: AuthenticatedMember) -> Result<Json<Vec<ThreadSummary>>, ApiError> {
    let rows = sqlx::query_as::<_, ThreadSummary>(r#"
...
pub async fn create_thread(State(state): State<AppState>, member: AuthenticatedMember, Json(input): Json<CreateThreadInput>) -> Result<Json<serde_json::Value>, ApiError> {
    if input.other_member_id == member.member_id { return Err(ApiError::invalid("otherMemberId")); }
    let active = sqlx::query_scalar::<_, bool>("select exists(select 1 from public.members where id = $1 and status = 'active')")
        .bind(input.other_member_id).fetch_one(&state.pool).await?;
    if !active { return Err(ApiError::NotFound("recipient")); }
    ...
    let thread_id = match existing {
        ...
        None => {
            let id = Uuid::new_v4();
            sqlx::query("insert into public.direct_threads (id) values ($1)").bind(id).execute(&mut *tx).await?;
...
pub async fn list_messages(State(state): State<AppState>, member: AuthenticatedMember, Path(thread_id): Path<Uuid>) -> Result<Json<Vec<MessageView>>, ApiError> {
    ...
    "#).bind(thread_id).fetch_all(&state.pool).await?;

    let mut result = Vec::with_capacity(rows.len());
    for row in rows {
        let mut attachments = sqlx::query_as::<_, AttachmentView>(r#"
        ...
        "#).bind(row.id).fetch_all(&state.pool).await?;
        for file in &mut attachments {
            file.url = signed_asset_url(&state, &file.file_path, 3600).await.unwrap_or_default();
        }
        let avatar_url = signed_asset_url(&state, &row.avatar_path, 28_800).await.unwrap_or_default();
        result.push(MessageView {
            id: row.id, thread_id: row.thread_id, author_id: row.author_id,
            body: row.body, created_at: row.created_at, edited_at: row.edited_at,
            reply_to: row.reply_to, is_pinned: row.is_pinned,
            author: AuthorView { id: row.author_id, name: row.author_name, email: row.author_email,
                avatar_path: row.avatar_path, avatar_url, job_title: row.job_title, job_roles: Vec::new() },
            attachments,
        });
    }
    Ok(Json(result))
...
pub async fn send_message(State(state): State<AppState>, member: AuthenticatedMember, Path(thread_id): Path<Uuid>, Json(input): Json<SendMessageInput>) -> Result<Json<serde_json::Value>, ApiError> {
    ensure_member(&state, thread_id, member.member_id).await?;
    let body = input.body.trim().chars().take(12_000).collect::<String>();
    if body.is_empty() { return Err(ApiError::invalid("body")); }
    let id = Uuid::new_v4();
    ...
    state.publish(BackendEvent::DirectMessageCreated { thread_id, message_id: id, author_id: member.member_id });
    Ok(Json(serde_json::json!({"id": id, "threadId": thread_id})))
}

pub async fn update_message(State(state): State<AppState>, member: AuthenticatedMember, Path(message_id): Path<Uuid>, Json(input): Json<UpdateMessageInput>) -> Result<Json<serde_json::Value>, ApiError> {
    let body = input.body.map(|value| value.trim().chars().take(12_000).collect::<String>());
    if body.as_ref().is_some_and(String::is_empty) { return Err(ApiError::invalid("body")); }
    ...
    state.publish(BackendEvent::DirectMessageUpdated { thread_id, message_id });
    Ok(Json(serde_json::json!({"ok": true})))
}

pub async fn delete_message(State(state): State<AppState>, member: AuthenticatedMember, Path(message_id): Path<Uuid>) -> Result<Json<serde_json::Value>, ApiError> {
    let thread_id = sqlx::query_scalar::<_, Uuid>("delete from public.direct_messages where id=$1 and author_id=$2 returning thread_id")
        .bind(message_id).bind(member.member_id).fetch_optional(&state.pool).await?
        .ok_or(ApiError::NotFound("message"))?;
    state.publish(BackendEvent::DirectMessageDeleted { thread_id, message_id });
    Ok(Json(serde_json::json!({"ok": true})))
}

pub async fn mark_read(State(state): State<AppState>, member: AuthenticatedMember, Path(thread_id): Path<Uuid>) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    if result.rows_affected() == 0 { return Err(ApiError::NotFound("thread")); }
    Ok(Json(serde_json::json!({"ok": true})))
}

pub async fn ensure_member(state: &AppState, thread_id: Uuid, member_id: Uuid) -> Result<(), ApiError> {
