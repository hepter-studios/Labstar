...
pub async fn create_call(
    ...
    .await?;
    if busy {
        return Err(ApiError::Conflict("participant_already_in_call"));
    }
    ...
    sqlx::query("insert into public.direct_call_sessions (id,thread_id,initiator_id,recipient_id,kind,status) values ($1,$2,$3,$4,$5,'ringing')")
        ...
        .execute(&mut *tx).await?;
    tx.commit().await?;
    state.publish(BackendEvent::CallCreated {
        call_id: id,
...
pub async fn pending_calls(
    ...
    let calls = sqlx::query_as::<_, CallSession>(
        r#"
...
pub async fn set_status(
    ...
    if matches!(input.status.as_str(), "accepted" | "rejected")
        && member.member_id != call.recipient_id
    {
    ...
    state.publish(BackendEvent::CallUpdated {
        call_id,
        status: input.status,
    });
...
pub async fn send_signal(
    ...
    if serde_json::to_vec(&payload)
        .map_err(|_| ApiError::invalid("payload"))?
        .len()
        > 65_536
    {
    ...
    ensure_participant(&call, member.member_id)?;
    if input.recipient_id == member.member_id
        || ![call.initiator_id, call.recipient_id].contains(&input.recipient_id)
    {
        return Err(ApiError::PermissionDenied);
    }
    ...
    .await?;
    state.publish(BackendEvent::CallSignal {
        call_id,
        signal_id,
        recipient_id: input.recipient_id,
    });
...
pub async fn list_signals(
    ...
    let call = load_call(&state, call_id).await?;
    ensure_participant(&call, member.member_id)?;
    let signals = sqlx::query_as::<_, CallSignal>(
        r#"
...
fn ensure_participant(call: &CallSession, member_id: Uuid) -> Result<(), ApiError> {
    [call.initiator_id, call.recipient_id]
        .contains(&member_id)
        .then_some(())
